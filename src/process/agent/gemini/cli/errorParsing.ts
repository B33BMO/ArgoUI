...
function isProQuotaExceededError(error: unknown): boolean {
  ...
  return (
    ...
    (errorStr.includes('pro') || errorStr.includes('2.5') || errorStr.includes('gemini-pro'))
  );
}
...
function isGenericQuotaExceededError(error: unknown): boolean {
  ...
  return (
    errorStr.includes('quota') &&
    (errorStr.includes('exceeded') || errorStr.includes('exhausted') || errorStr.includes('limit'))
  );
}
...
const getRateLimitErrorMessageGooglePaid = (fallbackModel: string = DEFAULT_GEMINI_FLASH_MODEL) =>
...
const getRateLimitErrorMessageGoogleGenericQuotaPaid = (currentModel: string = DEFAULT_GEMINI_MODEL) =>
