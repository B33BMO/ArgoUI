...
export class GeminiAgent {
  ...
  private toolConfig: ConversationToolConfig;
  ...
  private async initialize(): Promise<void> {
    ...
    // aioncli-core SkillManager.discoverSkills() skills skills
    ...
    if (this.authType === AuthType.LOGIN_WITH_GOOGLE) {
      ...
      if (!hasGoogleOAuthCredentials()) {
        ...
        // "authentication" GeminiSendBox API
  ...
  private initToolScheduler(_settings: Settings) {
    this.scheduler = new CoreToolScheduler({
        ...
        try {
          if (completedToolCalls.length > 0) {
            const refreshMemory = async () => {
              // aioncli-core refreshServerHierarchicalMemory
              // Directly use refreshServerHierarchicalMemory from aioncli-core
              // config ExtensionLoader memory
      ...
      // callback was removed in aioncli-core v0.18.4
  ...
  async injectConversationHistory(text: string): Promise<void> {
    try {
      ...
      // refreshServerHierarchicalMemory memory
