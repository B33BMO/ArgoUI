...
// Alert Alert component style constant
const ALERT_CLASSES =
...
// CollapsibleContent CollapsibleContent height constants
...
const ImageDisplay: React.FC<{
  ...
  // base64 Load local paths as base64
  React.useEffect(() => {
  ...
  // blobGet image blob (reusable logic)
  ...
  return (
    <>
      ...
      <div className='flex flex-col gap-8px my-8px' style={{ maxWidth: '197px' }}>
        {/*Image preview - PreviewGroup PreviewGroup*/}
  ...
  return (
    <div>
      {message.content.map((content, index) => {
        ...
        // ImageGeneration Alert Special handling for ImageGeneration: display image separately without Alert wrapper
        if (name === 'ImageGeneration' && typeof resultDisplay === 'object') {
          const result = resultDisplay as ImageGenerationResult;
          if (result.img_url) {
          ...
          <div key={callId}>
            ...
            {(description || resultDisplay || status === 'Error') && (
              <div className='mt-8px'>
                ...
                {resultDisplay && (
                  <div>
                    {/*Alert Display full result outside Alert*/}
                    {}
