...
describe('deepLink module', () => {
  ...
  describe('parseDeepLinkUrl', () => {
    it('should parse simple deep link URL', async () => {
      ...
      const result = parseDeepLinkUrl('argoui://add-provider?baseUrl=http://localhost&apiKey=sk-123');
    ...
    it('should parse deep link with path segments', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
      const result = parseDeepLinkUrl('argoui://provider/add?v=1');
    ...
    it('should decode base64 data param and merge into params', async () => {
      ...
      const result = parseDeepLinkUrl(`argoui://provider/add?v=1&data=${data}`);
    ...
    it('should handle invalid base64 data gracefully', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
      const result = parseDeepLinkUrl('argoui://add?data=not-valid-base64!!!');

      expect(result).not.toBeNull();
      expect(result!.params.data).toBeUndefined();
    });

    it('should return null for non-argoui protocol', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
  ...
  describe('handleDeepLinkUrl', () => {
    it('should queue URL when no window is set', async () => {
      ...
      handleDeepLinkUrl('argoui://test-action?key=val');

      expect(getPendingDeepLinkUrl()).toBe('argoui://test-action?key=val');
    });
  ...
  describe('pending URL state', () => {
    it('should clear pending URL', async () => {
      ...
      handleDeepLinkUrl('argoui://test');
      expect(getPendingDeepLinkUrl()).toBe('argoui://test');
